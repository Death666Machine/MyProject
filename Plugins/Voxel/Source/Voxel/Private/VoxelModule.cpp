...
#if USE_EMBREE_VOXEL
	const FString EmbreeBase = FPaths::Combine(*PluginBaseDir, TEXT("Source/ThirdParty/Embree3/embree-3.5.2/"));

#if PLATFORM_WINDOWS
	const FString EmbreeDllFolderPath = FPaths::Combine(*EmbreeBase, TEXT("/Win64/lib/"));
	const FString EmbreeDllName = "embree3.dll";
#elif PLATFORM_MAC
	const FString EmbreeDllFolderPath = FPaths::Combine(*EmbreeBase, TEXT("/MacOSX/lib/"));
	const FString EmbreeDllName = "libembree3.3.dylib";
#endif	//PLATFORM_WINDOWS

#if PLATFORM_WINDOWS || PLATFORM_MAC
	FPlatformProcess::PushDllDirectory(*EmbreeDllFolderPath);
	if (!FPlatformProcess::GetDllHandle(*EmbreeDllName))
	{
		FMessageDialog::Open(EAppMsgType::Ok, EAppReturnType::Ok, FText::Format(
			NSLOCTEXT(
				"Voxel",
				"EmbreeDllNotFound",
				"Voxel Plugin: {0} not found, voxel spawners won't work.\n"
				"Path: {1}"), FText::FromString(EmbreeDllName), FText::FromString(EmbreeDllFolderPath)));
	}
	FPlatformProcess::PopDllDirectory(*EmbreeDllFolderPath);
#endif
#endif
}
