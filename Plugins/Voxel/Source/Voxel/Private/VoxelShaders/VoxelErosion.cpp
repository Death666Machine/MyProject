...
FVoxelFloatTexture UVoxelErosion::GetTerrainHeightTexture()
{
	...
	ensureAlways(Texture->SizeX == RealSize && Texture->SizeY == RealSize);
...
void UVoxelErosion::RunShader(const FVoxelErosionParameters& Parameters)
{
	...
	RHICmdList.DispatchComputeShader(RealSize / NUM_THREADS_CS, RealSize / NUM_THREADS_CS, 1);
...
void UVoxelErosion::CopyRHIToTexture_RenderThread(const FTexture2DRHIRef& RHITexture, TVoxelTexture<float>::FTextureData& Texture)
{
	...
	Texture.TextureData.SetNumUninitialized(Size * Size);

	float* RESTRICT const Dest = Texture.TextureData.GetData();
	Texture.Min = RHIData[0];
	Texture.Max = RHIData[0];
	for (int32 Index = 0; Index < Size * Size; Index++)
	{
		Dest[Index] = RHIData[Index];
		Texture.Min = FMath::Min(Texture.Min, RHIData[Index]);
		Texture.Max = FMath::Max(Texture.Max, RHIData[Index]);
	}
