...
bool FVoxelMultiplayerTcpClient::Connect(const FString& Ip, int32 Port, FString& OutError)
{
	...
	UE_LOG(LogVoxel, Log, TEXT("TCP Client: Connected! %s:%d"), *Ip, Port);
...
bool FVoxelMultiplayerTcpServer::Start(const FString& Ip, int32 Port, FString& OutError)
{
	...
	UE_LOG(LogVoxel, Log, TEXT("TCP Server: Started! %s:%d"), *Ip, Port);
...
bool FVoxelMultiplayerTcpServer::Accept(FSocket* NewSocket, const FIPv4Endpoint& Endpoint)
{
	...
	UE_LOG(LogVoxel, Log, TEXT("TCP Server: Client connected! %s"), *Endpoint.ToString());

	AsyncTask(ENamedThreads::GameThread, FVoxelUtilities::MakeVoxelWeakPtrLambda(this, [](FVoxelMultiplayerTcpServer& This) { This.OnConnection.ExecuteIfBound(); }));
