...
void AVoxelWorld::SetOffset(const FIntVector& OffsetInVoxels)
{
	...
	*WorldOffset = OffsetInVoxels;
	...
	if (InstancedMeshManager.IsValid())
	{
...
{
	VOXEL_FUNCTION_COUNTER();
	return FVoxelInstancedMeshManager::Create(FVoxelInstancedMeshManagerSettings(this, Pool.ToSharedRef(), ProcGenManager.ToSharedRef()));
}

TVoxelSharedRef<FVoxelSpawnerManager> AVoxelWorld::CreateSpawnerManager() const
{
	VOXEL_FUNCTION_COUNTER();
