...
void FVoxelData::GetDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& OutValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& OutMaterialDiffQueue)
{
	...
	FVoxelReadScopeLock Lock(*this, FIntBox::Infinite, FUNCTION_FNAME);
	FVoxelOctreeUtilities::IterateAllLeaves(GetOctree(), [&](FVoxelDataOctreeLeaf& Leaf)
	{
		...
		if (Leaf.Multiplayer.IsValid())
		{
			if (Leaf.Multiplayer->IsNetworkDirty<FVoxelValue>())
			{
				...
				auto& Diff = *new (OutValueDiffQueue) TVoxelChunkDiff<FVoxelValue>(Leaf.Position);
				Leaf.Multiplayer->AddToDiffQueueAndReset(Leaf.Values.GetDataPtr(), Diff.Diffs);
			}
			if (Leaf.Multiplayer->IsNetworkDirty<FVoxelMaterial>())
			{
				if (Leaf.Materials.IsSingleValue()) Leaf.Materials.ExpandSingleValue();
				auto& Diff = *new (OutMaterialDiffQueue) TVoxelChunkDiff<FVoxelMaterial>(Leaf.Position);
				Leaf.Multiplayer->AddToDiffQueueAndReset(Leaf.Materials.GetDataPtr(), Diff.Diffs);
			}
		}
	});
}
...
template<typename T>
inline void LoadFromDiffsImpl(FVoxelData& Data, TArray<TVoxelChunkDiff<T>>& Queue, TArray<FIntBox>& OutModifiedBounds)
{
	VOXEL_FUNCTION_COUNTER();

	for (auto& ChunkDiff : Queue)
	{
		auto Bounds = FIntBox(ChunkDiff.Position - FIntVector(DATA_CHUNK_SIZE / 2), ChunkDiff.Position + FIntVector(DATA_CHUNK_SIZE / 2));
		...
		Leaf.InitForEdit<T>(*Data.WorldGenerator, Data.bEnableMultiplayer, Data.bEnableUndoRedo);
...
void FVoxelData::LoadFromDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& ValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& MaterialDiffQueue, TArray<FIntBox>& OutModifiedBounds)
{
	VOXEL_FUNCTION_COUNTER();
