...
FORCEINLINE uint32 GetIndex(const FIntBox& Box, const FIntVector& Size, int32 X, int32 Y, int32 Z)
{
...
FORCEINLINE uint32 GetIndex(const FIntBox& Box, const FIntVector& Size, const FIntVector& P)
{
	...
	FVoxelRemoveFloatingPartsResult& OutResult)
{
	...
	TArray<FVoxelMaterial> Materials;
	{
		...
		// Fill Visited array
		{
			VOXEL_SCOPE_COUNTER("Fill Visited array");
		...
		{
			VOXEL_SCOPE_COUNTER("Remove not visited voxels");
	...
	if (bDebug)
	{
		AsyncTask(ENamedThreads::GameThread, [FloatingPoints, DebugWorld]()
		{
	...
	{
		VOXEL_SCOPE_COUNTER("Create Parts");
		...
		if (bCreateData)
		{
			...
			const auto WorldGenerator = MakeVoxelShared<FVoxelEmptyWorldGeneratorInstance>(Data.WorldGenerator);
			...
			struct FNewPart
			{
				...
				FVoxelMutableDataAccelerator OctreeAccelerator;

				FNewPart(
					const FIntVector& NewPartCenter,
					uint8 InDepth,
					const TVoxelSharedRef<FVoxelWorldGeneratorInstance>& InWorldGenerator,
					bool bEnableMultiplayer,
					bool bEnableUndoRedo)
					: NewPartCenter(NewPartCenter)
					, NewPartData(FVoxelData::Create(FVoxelDataSettings(InDepth, InWorldGenerator, bEnableMultiplayer, bEnableUndoRedo)))
					, Lock(MakeUnique<FVoxelWriteScopeLock>(*NewPartData, FIntBox::Infinite, STATIC_FNAME("FloatingBlocks")))  // Make checks happy
					, OctreeAccelerator(*NewPartData, FIntBox::Infinite)
				{
				}
			};

			const auto InitNewPart = [Depth, &WorldGenerator, &Data](const FIntVector& NewPartCenter)
			{
				return FNewPart(NewPartCenter, Depth, WorldGenerator, Data.bEnableMultiplayer, Data.bEnableUndoRedo);
			};
...
void UVoxelPhysicsTools::SpawnFloatingPartsAndUpdateWorld(
	...
	FVoxelRemoveFloatingPartsResult&& RemoveFloatingPartsResult)
{
	...
	for (auto& NewPart : RemoveFloatingPartsResult.Parts)
	{
		...
		PartSpawner.SpawnPart(OnWorldUpdateDone, World, MoveTemp(NewPart.Data), MoveTemp(NewPart.Voxels), NewPart.PartCenter);
	...
	if (!ensure(RemoveFloatingPartsResult.BoxToUpdate.IsValid())) return;
	...
	bool bHideLatentWarnings)
{
	...
	FVoxelToolHelpers::StartAsyncLatentActionImpl<FWork>(
		...
		[&]()
	{
		return MakeVoxelShared<FWork>(
			FUNCTION_FNAME,
			World,
			[=](FVoxelData& Data, FVoxelRemoveFloatingPartsResult& Result)
		{
			RemoveFloatingParts(Data, Bounds, MinParts, bCreateData, bCreateVoxels, bDebug, VoxelWorldWeakPtr, Result);
		});
	},
		[=]
	(FWork& Work)
	{
		...
		{
			SpawnFloatingPartsAndUpdateWorld(*IPartSpawner, VoxelWorldWeakPtr.Get(), MoveTemp(Work.Value));
		}
	});
}