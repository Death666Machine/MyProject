	...
	bool bConvertToVoxelSpace)
{
	...
	EditPosition = GET_VOXEL_TOOL_REAL(EditPosition);

	TArray<FSurfaceVoxelWithStrength> NewVoxels = Voxels;
	FVector OutPlaneX, OutPlaneY;
	GetStrengthMaskBasisImpl(PlaneNormal, PlaneTangent, OutPlaneX, OutPlaneY);
	ApplyStrengthMaskImpl(NewVoxels, Mask.Texture, EditPosition, ScaleX, ScaleY, OutPlaneX, OutPlaneY, SamplerMode);
	return NewVoxels;
}
	...
	int32 ImmutableVoxels)
{
	...
	TArray<FSurfaceVoxelWithStrength> NewVoxels = Voxels;
	ApplyTerraceImpl(NewVoxels, TerraceHeightInVoxels, Angle, ImmutableVoxels);
	return NewVoxels;
}
