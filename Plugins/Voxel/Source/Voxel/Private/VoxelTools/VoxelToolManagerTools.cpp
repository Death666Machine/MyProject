...
void FVoxelToolManagerTool_Surface::Tick(AVoxelWorld& World, const FVoxelToolManagerTickData& TickData)
{
	...
	{
		FString Error;
		if (!FVoxelTextureUtilities::CanCreateFromTexture(SurfaceSettings.Mask, Error))
		{
			FVoxelMessages::ShowNotification(
				...
				"Fix Now",
				"Fix the texture",
				FSimpleDelegate::CreateWeakLambda(SurfaceSettings.Mask, [Mask = SurfaceSettings.Mask]() { FVoxelTextureUtilities::FixTexture(Mask); }));
			return;
		}
		MaskData.MaskTexture = FVoxelTextureUtilities::CreateFromTexture_Float(SurfaceSettings.Mask, SurfaceSettings.MaskChannel);
		const float BaseMaskScale = RadiusWithFalloff * 2 / World.VoxelSize * SurfaceSettings.MaskScale;
		MaskData.MaskScaleX = BaseMaskScale / MaskData.MaskTexture.GetSizeX();
		MaskData.MaskScaleY = BaseMaskScale / MaskData.MaskTexture.GetSizeY() * SurfaceSettings.MaskRatio;
		...
		MaskChannelVector[int32(SurfaceSettings.MaskChannel)] = 1;
		MaterialInstance.SetVectorParameterValue(STATIC_FNAME("MaskChannel"), FLinearColor(MaskChannelVector));
		MaterialInstance.SetTextureParameterValue(STATIC_FNAME("MaskTexture"), SurfaceSettings.Mask);
...
void FVoxelToolManagerTool_Mesh::Tick(AVoxelWorld& World, const FVoxelToolManagerTickData& TickData)
{
	...
	if (MeshDataStaticMesh != MeshSettings.Mesh)
	{
		MeshData.Reset();
		AssetData.Reset();
	}
	if (!MeshData.IsValid())
	{
		MeshDataStaticMesh = MeshSettings.Mesh;
		MeshData = MakeUnique<FVoxelMeshImporterInputData>();
		UVoxelMeshImporterLibrary::CreateMeshDataFromStaticMesh(MeshSettings.Mesh, *MeshData);

		MeshDataBounds = FBox(ForceInit);
		for (auto& Vertex : MeshData->Vertices)
		{
			MeshDataBounds += Vertex;
		}
	}
	...
	const FVector Scale =
		...
		? MeshSettings.Scale
		: ToolManager.Radius * MeshSettings.Scale / MeshDataBounds.GetSize().GetMax();
	...
	const FVector PositionOffset = MeshSettings.PositionOffset * ScaleMatrix.TransformVector(MeshDataBounds.GetSize());
	...
	const FTransform TransformNoTranslation = FTransform(
		...
		GetRotationMatrix());
	const FTransform TransformWithTranslation = TransformNoTranslation * FTransform(ToolPosition);
	...
	const bool bSetSingleIndex = MeshSettings.bSetIndex && World.MaterialConfig == EVoxelMaterialConfig::SingleIndex;
	const bool bSetDoubleIndex = MeshSettings.bSetIndex && World.MaterialConfig == EVoxelMaterialConfig::DoubleIndex;

	if (!TransformNoTranslation.Equals(AssetDataTransform))
	{
		AssetData.Reset();
	}
	if (!AssetData.IsValid())
	{
		InvertedAssetData.Reset();

		AssetDataTransform = TransformNoTranslation;
		...
		MeshImporterSettings.bPaintColors = MeshSettings.bPaintColors && MeshSettings.ColorsMaterial;
		MeshImporterSettings.ColorsMaterial = MeshSettings.ColorsMaterial;

		MeshImporterSettings.bPaintUVs = MeshSettings.bPaintUVs && MeshSettings.UVsMaterial;
		...
		MeshImporterSettings.SingleIndex = MeshSettings.PaintIndex;
		MeshImporterSettings.DoubleIndex = MeshSettings.PaintIndex;

		AssetData = MakeUnique<FVoxelDataAssetData>(nullptr);
		int32 NumLeaks;
	...
	const FVector VoxelPosition = World.GlobalToLocalFloat(ToolPosition) + FVector(AssetDataPositionOffset);
	const FIntBox Bounds = FIntBox(VoxelPosition, VoxelPosition + FVector(AssetData->GetSize()));
	const auto BoundsToCache = GetAndDebugBoundsToCache(World, Bounds, TickData);

	{
		...
		if (TickData.bAlternativeMode)
		{
			...
			{
				if (!InvertedAssetData)
				{
					InvertedAssetData = MakeUnique<FVoxelDataAssetData>(nullptr);
					UVoxelAssetTools::InvertDataAssetImpl(*AssetData, *InvertedAssetData);
				}
