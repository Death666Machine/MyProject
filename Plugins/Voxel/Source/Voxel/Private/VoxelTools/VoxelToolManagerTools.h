...
class FVoxelToolManagerTool_Mesh : public FVoxelToolManagerTool
{
public:
	const FVoxelToolManager_MeshSettings& MeshSettings;

	explicit FVoxelToolManagerTool_Mesh(const UVoxelToolManager& ToolManager);
	~FVoxelToolManagerTool_Mesh();
...
private:
	TWeakObjectPtr<UStaticMesh> MeshDataStaticMesh;
	TUniquePtr<FVoxelMeshImporterInputData> MeshData;
	FBox MeshDataBounds;

	FTransform AssetDataTransform;
	TUniquePtr<FVoxelDataAssetData> AssetData;
	TUniquePtr<FVoxelDataAssetData> InvertedAssetData;
	FIntVector AssetDataPositionOffset = FIntVector::ZeroValue;

	TUniquePtr<FVoxelMeshImporterRenderTargetCache> RenderTargetCache;
};
