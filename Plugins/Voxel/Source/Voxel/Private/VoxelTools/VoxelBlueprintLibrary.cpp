	...
	EVoxelSpawnerActorSpawnType SpawnType)
{
	...
	if (auto* MeshManager = World->GetInstancedMeshManager())
	{
		OutActors = MeshManager->SpawnActorsInArea(Bounds, World->GetData(), SpawnType);
	}
}
	...
	const FVector FloatingDetectionOffset)
{
	...
	if (auto* MeshManager = World->GetInstancedMeshManager())
	{
		...
		Matrices.Reserve(Transforms.Num());
		for (int32 Index = 0; Index < Transforms.Num(); Index++)
		{
			FVoxelSpawnerMatrix Matrix(Transforms[Index].ToMatrixWithScale());
			if (Colors.IsValidIndex(Index))
			{
				const FVoxelMaterial Material = FVoxelMaterial::CreateFromColor(Colors[Index]);
				Matrix.SetRandomInstanceId(Material.GetPackedInt());
			}
			Matrix.SetPositionOffset(-FloatingDetectionOffset);
			Matrices.Add(Matrix);
		}

		InstanceSettings.Mesh = Mesh;
		InstanceSettings.ActorTemplate = ActorTemplate;
		InstanceSettings.CullDistance.Min = StartCullDistance;
		InstanceSettings.CullDistance.Max = EndCullDistance;
		MeshManager->AddInstances(InstanceSettings, Matrices, FIntBox::Infinite);
	}
}
