...
public:
	...
	const FRotationMatrix RotationOffset;

	FVoxelBasicSpawnerProxy(UVoxelBasicSpawner* Spawner, FVoxelSpawnerManager& Manager, uint32 Seed = 0);

	inline bool CanSpawn(const FVector& Normal, const FVector& WorldUp) const
	{
		const float Angle = FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(Normal, WorldUp)));
		return GroundSlopeAngle.Contains(Angle);
	}
	inline FMatrix GetMatrixWithoutOffsets(const FRandomStream& Stream, const FVector& Normal, const FVector& WorldUp) const
	{
		...
		FMatrix Matrix = FScaleRotationTranslationMatrix(Scale, FRotator(Pitch, Yaw, 0.0f), FVector::ZeroVector);
		...
		case EVoxelBasicSpawnerRotation::AlignToWorldUp:
			...
			break;
		case EVoxelBasicSpawnerRotation::DoNotAlign:
