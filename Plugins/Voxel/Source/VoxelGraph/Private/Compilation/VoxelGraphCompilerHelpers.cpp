	...
	TMap<FVoxelCompilationNode*, FVoxelCompilationNode*>& OldNodesToNewNodes)
{
	...
	for (auto& It : Compiler.DuplicateCounts)
	{
		if (It.Key && It.Value > 8)
		{
			...
			Compiler.ErrorReporter.AddMessageToNode(It.Key, FString::Printf(TEXT("duplicated %d times: function separator needed"), It.Value), EVoxelGraphNodeMessageType::FatalError);
		}
	}
