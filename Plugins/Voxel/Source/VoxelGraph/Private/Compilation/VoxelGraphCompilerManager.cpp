...
bool FVoxelGraphCompilerManager::Compile(FVoxelCompiledGraphs& OutGraphs)
{
	...
	return !Compiler.ErrorReporter.HasFatalError();
}

#define CHECK_NO_ERRORS() if (Compiler.ErrorReporter.HasFatalError()) return false;
...
bool FVoxelGraphCompilerManager::CompileInternal(FVoxelGraphCompiler& Compiler, FVoxelCompiledGraphs& OutGraphs)
{
	...
	Compiler.ApplyPass<FVoxelGraphInlineMacrosPass>(/*bClearMessages*/ !bShowAxisDependencies);
...
struct FVoxelCompilationFunction
{
	...
	void PreCompile()
	{
		...
		Manager.Optimize(*Compiler, true);
...
bool FVoxelGraphCompilerManager::CompileOutput(const FVoxelGraphPermutationArray& Permutation, const FString& OutputName, FVoxelGraphCompiler& Compiler, TVoxelSharedPtr<FVoxelGraph>& OutGraph)
{
	...
	if (bPreview)
	{
		...
		Constants.Add("Preview.ResolutionScale", LexToString(PreviewSettings->ResolutionScale));
		...
		Constants.Add("Preview.MinValue", LexToString(PreviewSettings->MinValue));
		Constants.Add("Preview.MaxValue", LexToString(PreviewSettings->MaxValue));
	...
	{
		Compiler.ApplyPass<FVoxelDisconnectStaticClampInputsPass>();
		Compiler.ApplyPass<FVoxelRemoveAllSeedNodesPass>();
	}

	CHECK_NO_ERRORS();

	Optimize(Compiler, false);
	...
	for (auto& FunctionDescriptor : FunctionsDescriptors)
	{
		...
		if (FunctionCompiler->ErrorReporter.HasFatalError())
		{
			return false;
		}
	...
	if (bShowAxisDependencies && (bDebug || Graph->bDetailedErrors))
	{
		for (auto& Function : Functions)
		{
	...
	OutGraph = MakeShareable(new FVoxelGraph(
		...
		Id,
		Graph->bEnableStats,
		Graph->bEnableRangeAnalysisDebug && bPreview));
	...
	if (FlowMergeOrFunction)
	{
		if (Graph->bShowFlowMergeAndFunctionsWarnings)
		{
			const auto Result = FMessageDialog::Open(EAppMsgType::YesNo,
				LOCTEXT("FlowMergeFunctionWarning",
...
void FVoxelGraphCompilerManager::Optimize(FVoxelGraphCompiler& Compiler, bool bIsFunction) const
{
	VOXEL_FUNCTION_COUNTER();
	...
	while (bContinue)
	{
		...
		if (!bIsFunction)
		{
			Compiler.ApplyPass<FVoxelRemoveUnusedFunctionPinsPass>(bContinue); // Won't compile without, as function separator output ids would be -1
		}
		...
		if (Compiler.ErrorReporter.HasFatalError())
		{
			return;
		}
	}
