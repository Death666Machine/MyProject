...
void FVoxelReplaceBiomeMergePass::Apply(FVoxelGraphCompiler& Compiler)
{
	for (auto* Node : Compiler.GetAllNodesCopy())
	{
		...
		{
			const int32 AdditionalDataNum = BiomeMergeNode->GetOutputCount() - 2;
			ensure((BiomeMergeNode->GetInputCount() - 1 - AdditionalDataNum) % 2 == 0);
			const int32 BiomesNum = (BiomeMergeNode->GetInputCount() - 1 - AdditionalDataNum) / 2;
			...
			TArray<FVoxelCompilationPin*> AdditionalDataInputPins;
			for (int32 Index = 0; Index < AdditionalDataNum; Index++)
			{
				auto& InputPin = BiomeMergeNode->GetInputPin(1 + 2 * BiomesNum + Index);
				auto* Passthrough = FVoxelGraphCompilerHelpers::AddPassthrough(Compiler, InputPin, BiomeMergeNode);
				AdditionalDataInputPins.Add(&Passthrough->GetOutputPin(0));
			}

			TArray<FVoxelCompilationPin*> AdditionalDataOutputPins;
			TArray<FVoxelCompilationNode*> AdditionalDataOutputPassthroughs;
			for (int32 Index = 0; Index < AdditionalDataNum; Index++)
			{
				auto& OutputPin = BiomeMergeNode->GetOutputPin(2 + Index);
				auto* Passthrough = FVoxelGraphCompilerHelpers::AddPassthrough(Compiler, OutputPin, BiomeMergeNode);
				AdditionalDataOutputPins.Add(&Passthrough->GetInputPin(0));
				AdditionalDataOutputPassthroughs.Add(Passthrough);
			}
			...
			TArray<FVoxelCompilationPin*> LastAdditionalDataOutputs = AdditionalDataInputPins;
			...
			for (int32 BiomeIndex = 0; BiomeIndex < BiomesNum; BiomeIndex++)
			{
				AddMultiplyAdd(
					...
					*LastExecOutput,
					*LastSumOutput,
					*BiomesInputPins[2 * BiomeIndex],
					*LastBiomesOutputs[BiomeIndex],
					LastExecOutput,
					LastSumOutput);

				TArray<EVoxelPinCategory> SeparatorCategories;
				SeparatorCategories.Add(EVoxelPinCategory::Exec);
				SeparatorCategories.Add(EVoxelPinCategory::Float);
				for (int32 Index = BiomeIndex + 1; Index < BiomesNum; Index++)
				{
					SeparatorCategories.Add(EVoxelPinCategory::Float);
				}
				const int32 AdditionalDataOffset = SeparatorCategories.Num();
				for (auto* AdditionalDataInputPin : AdditionalDataInputPins)
				{
					SeparatorCategories.Add(AdditionalDataInputPin->PinCategory);
				}

				auto* FunctionSeparator = Compiler.AddNode(MakeShareable(
					new FVoxelFunctionSeparatorCompilationNode(
						*GetDefault<UVoxelNode>(),
						SeparatorCategories,
						SeparatorCategories)));
				FunctionSeparator->SourceNodes = BiomeMergeNode->SourceNodes;

				FunctionSeparator->GetInputPin(0).LinkTo(*LastExecOutput);
				FunctionSeparator->GetInputPin(1).LinkTo(*LastSumOutput);
				LastExecOutput = &FunctionSeparator->GetOutputPin(0);
				LastSumOutput = &FunctionSeparator->GetOutputPin(1);
				for (int32 Index = 0; Index < BiomesNum; Index++)
				{
					if (Index <= BiomeIndex)
					{
						LastBiomesOutputs[Index] = nullptr;
					}
					else
					{
						FunctionSeparator->GetInputPin(Index - BiomeIndex + 1).LinkTo(*LastBiomesOutputs[Index]);
						LastBiomesOutputs[Index] = &FunctionSeparator->GetOutputPin(Index - BiomeIndex + 1);
					}
				}
				for (int32 Index = 0; Index < AdditionalDataInputPins.Num(); Index++)
				{
					FunctionSeparator->GetInputPin(AdditionalDataOffset + Index).LinkTo(*LastAdditionalDataOutputs[Index]);
					LastAdditionalDataOutputs[Index] = &FunctionSeparator->GetOutputPin(AdditionalDataOffset + Index);
				}
			}
			...
			for (int32 Index = 0; Index < AdditionalDataInputPins.Num(); Index++)
			{
				AdditionalDataOutputPins[Index]->LinkTo(*LastAdditionalDataOutputs[Index]);
			}
			...
			OutputPassthroughs.Append(AdditionalDataOutputPassthroughs);
