...
inline void ReplaceFlowMerge(FVoxelGraphCompiler& Compiler, FVoxelFlowMergeCompilationNode* FlowMergeNode)
{
	...
	for (auto& Node : NodesToDuplicate)
	{
		...
		if (Node->IsExecNode() && !FVoxelGraphCompilerHelpers::AreAllNodePredecessorsChildOfStartNodeExecOnly(Node, FlowMergeNode, FaultyNode))
		{
			...
			Compiler.ErrorReporter.AddMessageToNode(FaultyNode, "should be child of flow merge", EVoxelGraphNodeMessageType::FatalError);
			return;
		}
	}
...
{
	bool bContinue = true;
	while (bContinue && !Compiler.ErrorReporter.HasFatalError())
	{
		bContinue = false;
	...
	if (!Compiler.ErrorReporter.HasFatalError())
	{
		...
		for (auto& Node : Nodes)
		{
			...
			{
				Compiler.ErrorReporter.AddMessageToNode(Node, "FlowMerge data output is used but exec output isn't connected", EVoxelGraphNodeMessageType::FatalError);
				break;
			}
		}
	}
