...
inline void ReplaceMacro(FVoxelGraphCompiler& Compiler, FVoxelMacroCompilationNode* MacroCompilationNode, bool bClearMessages)
{
	...
	if (StackMacros.Contains(MacroNode))
	{
		...
		ErrorReporter.AddMessageToNode(MacroCompilationNode, "recursive macros detected", EVoxelGraphNodeMessageType::FatalError);
		return;
	}
	...
	StackMacros.Add(MacroNode);
	{
		...
		// Merge macro nodes
		{
			...
			{
				ErrorReporter.AddMessageToNode(MacroNode, "errors in macro", EVoxelGraphNodeMessageType::FatalError);
			}
		}
		if (ErrorReporter.HasFatalError())
		{
