	...
	for (auto* Node : Compiler.GetAllNodesCopy())
	{
		...
		if (Constant)
		{
			...
			if (ensure(CompileTimeConstantNode))
			{
				...
				CompileTimeConstantNode->Constants.KeySort([](auto& A, auto& B) {return A < B; });
#else
...
void FVoxelRemoveUnusedFunctionPinsPass::Apply(FVoxelGraphCompiler& Compiler, bool& bChanged)
{
	for (auto* Node : Compiler.GetAllNodes())
	{
		if (Node->IsA<FVoxelFunctionSeparatorCompilationNode>() && Node != Compiler.FirstNode)
		{
			TArray<EVoxelPinCategory> Categories;
			bool bRecreateNode = false;
			for (auto& OutputPin : Node->IteratePins<EVoxelPinIter::Output>())
			{
				if (OutputPin.NumLinkedTo() == 0)
				{
					Node->GetInputPin(OutputPin.Index).BreakAllLinks();
					bRecreateNode = true;
				}
				else
				{
					Categories.Add(OutputPin.PinCategory);
				}
			}
			if (bRecreateNode)
			{
				bChanged = true;
				auto NewNode = MakeShared<FVoxelFunctionSeparatorCompilationNode>(
					Node->Node,
					Categories,
					Categories);

				int32 NewNodePinIndex = 0;
				for (auto& OldOutputPin : Node->IteratePins<EVoxelPinIter::Output>())
				{
					if (OldOutputPin.NumLinkedTo() != 0)
					{
						auto& NewOutputPin = NewNode->GetOutputPin(NewNodePinIndex);
						auto& NewInputPin = NewNode->GetInputPin(NewNodePinIndex);
						auto& OldInputPin = Node->GetInputPin(OldOutputPin.Index);
						FVoxelGraphCompilerHelpers::MovePin(OldInputPin, NewInputPin);
						FVoxelGraphCompilerHelpers::MovePin(OldOutputPin, NewOutputPin);
						NewNodePinIndex++;
					}
				}

				Node->CheckIsNotLinked(Compiler.ErrorReporter);
				Compiler.RemoveNode(Node);
