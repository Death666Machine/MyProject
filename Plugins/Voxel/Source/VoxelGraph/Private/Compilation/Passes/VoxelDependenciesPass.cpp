		...
		for (auto* Node : Compiler.GetAllNodes())
		{
			if (Node->IsA<FVoxelIfCompilationNode>())
			{
				...
				for (auto* Successor : Successors)
				{
					...
					{
						const uint8 OldDeps = Successor->Dependencies;
						Successor->Dependencies |= Node->Dependencies;
						bContinue |= (OldDeps != Successor->Dependencies);
					}
				}
			}
		}
...
void FVoxelDebugDependenciesPass::Apply(FVoxelGraphCompiler& Compiler)
{
	for (auto& Node : Compiler.GetAllNodes())
	{
		Compiler.ErrorReporter.AddMessageToNode(Node, FVoxelAxisDependencies::ToString(FVoxelAxisDependencies::GetVoxelAxisDependenciesFromFlag(Node->Dependencies)), EVoxelGraphNodeMessageType::Dependencies, false);
	}
}
