		...
		for (int32 PinIndex = 0; PinIndex < Node->InputPins.Num(); PinIndex++)
		{
			...
			for (int32 I = 0; I < NodeInputPin.OtherNodes.Num(); I++)
			{
				...
				if (!OtherNode)
				{
					...
					ErrorReporter.AddMessageToNode(CompilationNode, "An invalid node is linked to this node", EVoxelGraphNodeMessageType::FatalError);
					return {};
				}
				...
				{
					ErrorReporter.AddMessageToNode(OtherNode, "Internal error: Node not found", EVoxelGraphNodeMessageType::FatalError);
					return {};
				}
				...
				if (!CompilationNodeInputPin.IsLinkedTo(OtherPin))
				{
					...
					{
						ErrorReporter.AddMessageToNode(OtherNode, "Pins with different categories are linked together", EVoxelGraphNodeMessageType::FatalError);
						return {};
					}
...
FVoxelCompilationNode* FVoxelGraphCompiler::AddNode(const TSharedPtr<FVoxelCompilationNode>& Ref)
{
	NodesRefs.Add(Ref);
	Nodes.Add(Ref.Get());
	return Ref.Get();
