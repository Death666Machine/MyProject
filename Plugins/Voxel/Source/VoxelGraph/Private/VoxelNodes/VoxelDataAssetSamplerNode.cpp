...
TVoxelSharedPtr<FVoxelComputeNode> UVoxelNode_DataAssetSampler::GetComputeNode(const FVoxelCompilationNode& InCompilationNode) const
{
	...
	public:
		...
		void Compute(const FVoxelNodeType I[], FVoxelNodeType O[], const FVoxelContext& Context) const override
		{
		...
		void ComputeRange(const FVoxelNodeRangeType Inputs[], FVoxelNodeRangeType O[], const FVoxelContextRange& Context) const override
		{
		...
		void ComputeCpp(const TArray<FString>& I, const TArray<FString>& O, FVoxelCppConstructor& Constructor) const override
		{
			if (bBilinearInterpolation)
			{
				if (IsOutputUsed(0)) Constructor.AddLinef(TEXT("%s = %s->GetInterpolatedValue(%s, %s, %s, FVoxelValue::Empty());"), *O[0], *Variable->Name, *I[0], *I[1], *I[2]);
				if (IsOutputUsed(1)) Constructor.AddLinef(TEXT("%s = %s->GetInterpolatedMaterial(%s, %s, %s);"), *O[1], *Variable->Name, *I[0], *I[1], *I[2]);
			}
			else
			{
				if (IsOutputUsed(0)) Constructor.AddLinef(TEXT("%s = %s->GetValue(%s, %s, %s, FVoxelValue::Empty());"), *O[0], *Variable->Name, *I[0], *I[1], *I[2]);
				if (IsOutputUsed(1)) Constructor.AddLinef(TEXT("%s = %s->GetMaterial(%s, %s, %s);"), *O[1], *Variable->Name, *I[0], *I[1], *I[2]);
			}
			if (IsOutputUsed(2)) Constructor.AddLinef(TEXT("%s = %s->GetSize().X;"), *O[2], *Variable->Name);
			if (IsOutputUsed(3)) Constructor.AddLinef(TEXT("%s = %s->GetSize().Y;"), *O[3], *Variable->Name);
			if (IsOutputUsed(4)) Constructor.AddLinef(TEXT("%s = %s->GetSize().Z;"), *O[4], *Variable->Name);
		}
		void ComputeRangeCpp(const TArray<FString>& Inputs, const TArray<FString>& Outputs, FVoxelCppConstructor& Constructor) const override
		{
			...
			if (IsOutputUsed(2)) Constructor.AddLinef(TEXT("%s = %s->GetSize().X;"), *Outputs[2], *Variable->Name);
			if (IsOutputUsed(3)) Constructor.AddLinef(TEXT("%s = %s->GetSize().Y;"), *Outputs[3], *Variable->Name);
			if (IsOutputUsed(4)) Constructor.AddLinef(TEXT("%s = %s->GetSize().Z;"), *Outputs[4], *Variable->Name);
		}
		void SetupCpp(FVoxelCppConfig& Config) const override
		{
			Config.AddExposedVariable(Variable);
			Config.AddInclude("VoxelAssets/VoxelDataAsset.h");
		}
