...
void FVoxelGraphFunction::Init(const FVoxelWorldGeneratorInit& InitStruct, FVoxelGraphVMInitBuffers& Buffers) const
{
	check(IsUsedForInit());
...
void FVoxelGraphFunction::Compute(const FVoxelContext& Context, FVoxelGraphVMComputeBuffers& Buffers) const
{
	FunctionInit->CopyOutputsToVariables(Buffers.FunctionInputsOutputs, Buffers.Variables);
	const FVoxelGraphFunction* NextFunction = Tree->Compute<bEnableStats>(Context, Buffers);
	if (NextFunction)
	{
		NextFunction->Compute<bEnableStats>(Context, Buffers);
	}
}

template void FVoxelGraphFunction::Compute<false>(const FVoxelContext&, FVoxelGraphVMComputeBuffers&) const;
template void FVoxelGraphFunction::Compute<true>(const FVoxelContext&, FVoxelGraphVMComputeBuffers&) const;

template<bool bEnableRangeAnalysisDebug>
void FVoxelGraphFunction::ComputeRange(const FVoxelContextRange& Context, FVoxelGraphVMComputeRangeBuffers& Buffers) const
{
	FunctionInit->CopyOutputsToVariables(Buffers.FunctionInputsOutputs, Buffers.Variables);
	const FVoxelGraphFunction* NextFunction = Tree->ComputeRange<bEnableRangeAnalysisDebug>(Context, Buffers);
	if (NextFunction && !FVoxelRangeFailStatus::Get().HasFailed())
	{
		NextFunction->ComputeRange<bEnableRangeAnalysisDebug>(Context, Buffers);
	}
}

template void FVoxelGraphFunction::ComputeRange<false>(const FVoxelContextRange&, FVoxelGraphVMComputeRangeBuffers&) const;
template void FVoxelGraphFunction::ComputeRange<true>(const FVoxelContextRange&, FVoxelGraphVMComputeRangeBuffers&) const;
