...
virtual void ComputeStats() const override final \
{ \
	return FVoxelNodeHelpers::ComputeStatsImpl(this); \
} \
virtual void CacheFunctionPtr() override final \
{ \
	ComputeFunctionPtr = static_cast<FVoxelDataComputeNode::FComputeFunction>(&FLocalVoxelComputeNode::Compute); \
}

#define GENERATED_COMPUTE(InputsDef, OutputsDef, ...) \
void Compute(const FVoxelNodeType Inputs[], FVoxelNodeType Outputs[], const FVoxelContext& _C0) const override \
{ \
	InputsDef \
	OutputsDef \
	__VA_ARGS__ \
} \
void ComputeRange(const FVoxelNodeRangeType Inputs[], FVoxelNodeRangeType Outputs[], const FVoxelContextRange& _C0) const override \
{ \
	InputsDef \
	OutputsDef \
	__VA_ARGS__ \
