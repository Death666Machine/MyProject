...
class FVoxelComputeNodeWithCustomData
{
public:
	const TArray<FName> CustomData;
	const int32 NumDefaultInputPins;

	FVoxelComputeNodeWithCustomData(const TArray<FName>& CustomData, int32 NumDefaultInputPins);

	// Items: to copy custom data from the items one
	FVoxelGraphCustomData GetCustomData(const FVoxelNodeType Inputs[], const FVoxelItemStack* Items) const;
	FVoxelGraphCustomDataRange GetCustomData(const FVoxelNodeRangeType Inputs[], const FVoxelItemStack* Items) const;

	void GetCustomData(FVoxelCppConstructor& Constructor, const TArray<FString>& Inputs, bool bIsRange, bool bCopyFromItems) const;
};
