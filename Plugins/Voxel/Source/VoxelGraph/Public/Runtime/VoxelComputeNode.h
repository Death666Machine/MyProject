...
public:
	...
	inline int32 GetInputId(int32 Index) const { checkVoxelGraph(0 <= Index && Index < InputCount); return InputIds.GetData()[Index]; }
	inline int32 GetOutputId(int32 Index) const { checkVoxelGraph(0 <= Index && Index < OutputCount); return OutputIds.GetData()[Index]; }

	inline bool IsOutputUsed(int32 Index) const { return GetOutputId(Index) != -1; }

	inline EVoxelPinCategory GetInputCategory(int32 Index) const { return InputsCategories[Index]; }
	inline EVoxelPinCategory GetOutputCategory(int32 Index) const { return OutputsCategories[Index]; }
...
template<>
inline Seed FVoxelComputeNode::GetDefaultValue<Seed>(int32 Index) const { return DefaultValues[Index].Get<int32>(); }
...
public:
	...
	using FComputeFunction = void(FVoxelDataComputeNode::*)(const FVoxelNodeType[], FVoxelNodeType[], const FVoxelContext&) const;
	FComputeFunction ComputeFunctionPtr = nullptr;

	//~ Begin FVoxelDataComputeNode Interface
	virtual void Init(Seed Inputs[], const FVoxelWorldGeneratorInit& InitStruct) {};
	virtual void InitCpp(const TArray<FString>& Inputs, FVoxelCppConstructor& Constructor) const {}

	virtual void Compute(const FVoxelNodeType      Inputs[], FVoxelNodeType      Outputs[], const FVoxelContext     & Context) const = 0;
	virtual void ComputeRange(const FVoxelNodeRangeType Inputs[], FVoxelNodeRangeType Outputs[], const FVoxelContextRange& Context) const = 0;
...
public:
	...
	virtual void Init(Seed Inputs[], Seed Outputs[], const FVoxelWorldGeneratorInit& InitStruct) {};
...
public:
	...
	virtual void ComputeSetterNode(const FVoxelNodeType Inputs[], FVoxelNodeType GraphOutputs[]) const = 0;
	virtual void ComputeRangeSetterNode(const FVoxelNodeRangeType Inputs[], FVoxelNodeRangeType GraphOutputs[]) const = 0;
