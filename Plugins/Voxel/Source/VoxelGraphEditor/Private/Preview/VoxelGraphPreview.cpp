...
void FVoxelGraphPreview::Update(bool bUpdateTextures, bool bAutomaticPreview)
{
	...
	if (bUpdateTextures)
	{
		...
		{
			TVoxelSharedPtr<FVoxelGraphGeneratorInstance> WorldGeneratorInstance;
			{
				if (!WorldGenerator->GetGraphInstance(WorldGeneratorInstance, true, bAutomaticPreview))
				{
					return;
				}

				FVoxelWorldGeneratorInit Init(TMap<FName, int32>(), 100, Size.GetMax(), EVoxelMaterialConfig(-1) /* material warnings are annoying in preview */, nullptr, nullptr);
				WorldGeneratorInstance->Init(Init);

				//if (!bAutomaticPreview) // Slower, but else not up to date
				{
					// To have the range analysis error messages.
					WorldGeneratorInstance->GetValueRange(Settings->GetBounds(), 0, FVoxelItemStack::Empty);
				}
			}

			{
				TVoxelQueryZone<float> QueryZone(Settings->GetBounds(), Size, ResolutionScale, Values);
				WorldGeneratorInstance->GetOutput<false, float, float, FVoxelGraphOutputsIndices::ValueIndex>(
					FTransform(),
					1,
					QueryZone,
					Settings->LODToPreview,
					FVoxelItemStack::Empty,
					[](auto, auto, auto, auto) { return 0; });
			}
			if (bComputeMaterials)
			{
				TVoxelQueryZone<FVoxelMaterial> QueryZone(Settings->GetBounds(), Size, ResolutionScale, Materials);
				WorldGeneratorInstance->GetMaterials(QueryZone, Settings->LODToPreview, FVoxelItemStack::Empty);
			}
		}
