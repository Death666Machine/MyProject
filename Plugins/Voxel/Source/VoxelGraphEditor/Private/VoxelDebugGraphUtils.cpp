...
void FVoxelDebugGraphUtils::DebugNodes(const TSet<FVoxelCompilationNode*>& Nodes, UVoxelGraphGenerator* WorldGenerator, bool bShowAxisDependencies)
{
	...
	UEdGraph* const Graph = WorldGenerator->VoxelDebugGraph;
	Graph->Modify();
	...
	for (auto* CompilationNode : Nodes)
	{
		...
		if (WorldGenerator->bShowPinsIds)
		{
		...
		{
			GraphNode->NodePosX *= WorldGenerator->NodesDepthScaleFactor;
			GraphNode->NodePosY *= WorldGenerator->NodesDepthScaleFactor;
		...
		for (int32 I = 0; I < CompilationNode->GetOutputCount(); I++)
		{
			...
			for (auto& LinkedToPin : Pin.IterateLinkedTo())
			{
				...
				if (Nodes.Contains(&OtherNode) && (Pin.PinCategory == EVoxelPinCategory::Exec || !WorldGenerator->bHideDataNodes))
				{
