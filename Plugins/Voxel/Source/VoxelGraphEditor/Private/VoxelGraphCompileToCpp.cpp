...
static bool CompileInternal(const TSharedRef<SWidget>& Widget, UVoxelGraphGenerator* WorldGenerator, bool bIsAutomaticCompile, FString& OutError)
{
	...
	{
		auto Result = FMessageDialog::Open(EAppMsgType::OkCancel, LOCTEXT("ConvertToSourceProject",
	...
	{
		auto Result = FMessageDialog::Open(EAppMsgType::OkCancel, LOCTEXT("CreateGeneratedWorldGeneratorsDirectory",
	...
	// GeneratedWorldGenerators module dependency
	{
		...
		for (const auto& File : FoundFiles)
		{
			...
			{
				auto Result = FMessageDialog::Open(EAppMsgType::OkCancel, LOCTEXT("AddGeneratedWorldGeneratorsDependency",
	...
	// Show the file browser dialog
	{
		...
		{
			OutFiles.Add(WorldGenerator->SaveLocation.FilePath);
		}
		else
		{
			if (DesktopPlatform->SaveFileDialog(
				FSlateApplication::Get().FindBestParentWindowHandleForDialogs(Widget),
				"File to create",
				WorldGenerator->LastSavePath.IsEmpty() ? GeneratedDirectory : WorldGenerator->LastSavePath,
				WorldGenerator->GetName() + ".h",
				TEXT("C++ header (*.h)|*.h"),
		...
		if (OutFiles.Num() > 0)
		{
			...
			const FString HeaderPath = OutFiles[0];
			...
			FString HeaderText;
			FString CppText;
			if (!WorldGenerator->CompileToCpp(HeaderText, CppText, BaseName))
			{
			...
			WorldGenerator->LastSavePath = FolderPath;
...
void FVoxelGraphCompileToCpp::Compile(const TSharedRef<SWidget>& Widget, UVoxelGraphGenerator* WorldGenerator, bool bIsAutomaticCompile)
{
	FString Error;
	if (!CompileInternal(Widget, WorldGenerator, bIsAutomaticCompile, Error))
	{
		FMessageDialog::Open(EAppMsgType::Ok, FText::FromString("Error: " + Error));
	}
}
