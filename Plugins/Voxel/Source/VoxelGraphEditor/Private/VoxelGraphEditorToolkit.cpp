...
{
	FVoxelDebugGraphUtils::DebugNodes(Nodes, WorldGenerator, bShowAxisDependencies);
...
void FVoxelGraphEditorToolkit::SaveAsset_Execute()
{
	...
	{
		FVoxelGraphCompileToCpp::Compile(VoxelGraphEditor.ToSharedRef(), WorldGenerator, true);
	}
}
...
{
	FVoxelGraphCompileToCpp::Compile(VoxelGraphEditor.ToSharedRef(), WorldGenerator, false);
}
...
void FVoxelGraphEditorToolkit::ShowAxisDependencies()
{
	...
	if (bShowAxisDependencies)
	{
		...
		WorldGenerator->CreateGraphs(Graphs, false, false, true);
	}
}
