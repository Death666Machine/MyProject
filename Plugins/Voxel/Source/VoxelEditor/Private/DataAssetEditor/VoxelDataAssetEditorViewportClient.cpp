	...
	, Panel(Panel)
{
...

	{
		...
		const FIntBox Bounds = DataAsset.GetBounds();
		...
		Position[Axis] = FMath::Max(Size[(Axis + 1) % 3], Size[(Axis + 2) % 3]);
...
void FVoxelDataAssetEditorViewportClient::Tick(float DeltaSeconds)
{
	...
	for (auto* VoxelWorldEditor : TActorRange<AVoxelWorldEditor>(World))
	{
